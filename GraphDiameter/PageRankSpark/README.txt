Finding PageRanks for Custom Graph
and Finding diameter of Twitter Graph - Using Spark
CS6240 Fall 2018 HW3
=================================================
...
1) Clone this repository to your local machine by using the following command in git bash,
   and navigate into the HW2 folder.
...
5) Update the following variables in the Makefile to customize according to your setup:
    ...
    hdfs.user.name, 
    aws.bucket.name,
    app.name, (Set it to the program you wish to run)
    job.name (Set it to the program you wish to run)
...
An output folder is created with the output files inside it in the same /PageRankSpark/ directory.
